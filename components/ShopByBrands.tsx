...
const ShopByBrands = async () => {
    ...
    return (
        <div className="mb-10 lg:mb-20 bg-lightColor/9 p-5 lg:p-7 rounded-md">
            ...
            gap-2.5">
                {brands?.map((brand) => (
                    <Link
                        key={brand?._id}
                        href={{pathname: "/shop", query:{ brand: brand?.
                            slug?.current}}}
                    ...
                    >
                        {brand?.image && (
                            <Image
                                ...
                                alt="brandImage" // Use brand name for alt text
                                width={250}
                                height={250}
           ...
           gap-4 mt-16 p-2 shadow-sm hover:shadow-shop_light_green/20 py-5">
            {extraData?.map((item, index) =>(
                ...
                hover:text-shop_light_green/50"
                >
                <span className="inline-flex scale-100 
                group-hover:scale-90 hoverEffect">
                    {item?.icon}
                </span>
