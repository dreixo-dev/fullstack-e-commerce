...
const SideMenu: FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  className={`fixed inset-y-0 h-screen left-0 z-50 w-full
     bg-black/50 text-white/70 shadow-xl ${isOpen?"translate-x-0":"-translate-x-full"
      } hoverEffect`}>
        <div
        ref={sidebarRef} className="min-w-72 max-w-96 bg-black h-screen p-10
            ...
            font-semibold tracking-wide mt-1">
                {headerData?.map((item)=>(
        <Link 
        href={item?.href}
         key={item?.title}
