    ...
    return ( 
        <div className="hidden md:inline-flex w-1/3 items-center justify-center gap-7 text-sm capitalize font-semibold text-lightColor">
            {headerData?.map((item) => (
                ...
                >
                    {item?.title}
                    <span className={`absolute -bottom-0.5 left-1/2 h-0.5 bg-shop_light_green group-hover:w-1/2 hoverEffect group-hover:left-0 ${
                        pathname === item?.href ? "w-1/2 left-0" : "w-0"
                    }`} />
                    <span className={`absolute -bottom-0.5 right-1/2 h-0.5 bg-shop_light_green group-hover:w-1/2 hoverEffect group-hover:right-0 ${
